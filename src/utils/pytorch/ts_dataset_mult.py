...
class TimeSeriesDataset(Dataset):
    def __init__(self,
        ...
        ts_data, s_data, self.meta_data, self.n_y, self.t_cols, self.X_cols = self._df_to_lists(Y_df=Y_df, S_df=S_df, X_df=X_df)

        # Dataset attributes
        self.n_series   = len(ts_data)
        self.max_len    = max([len(list(ts.values())[0]) for ts in ts_data])
        self.n_channels = len(self.t_cols) # Y_cols, X_cols, insample_mask and outsample_mask
    ...
    def _df_to_lists(self, Y_df, S_df, X_df):
        ...
        for i, u_id in enumerate(unique_ids):
            ...
            # Y values
            for Y_col in Y_cols:
                serie =  Y_df[top_row:bottom_row][Y_col].values
                ts_data_i[Y_col] = serie
        ...
        t_cols = Y_cols + X_cols + ['insample_mask', 'outsample_mask']
        n_y = len(Y_cols)
    
        return ts_data, s_data, meta_data, n_y, t_cols, X_cols

    def _create_tensor(self, ts_data, s_data):
        """
        s_matrix of shape (n_series, n_s)
        ts_tensor of shape (n_series, n_channels, max_len) n_channels = Y_cols + X_cols + masks
        """
