...
class TimeSeriesLoader(object):
    ...
    def _create_windows_tensor(self):
        ...
        tensor[:, :self.ts_dataset.n_y, -self.output_size:] = 0 # overkill to ensure no validation leakage
    ...
    def _nbeats_batch(self, index):
        ...
        insample_y = windows[:, :self.ts_dataset.n_y, :self.input_size]
        insample_x = windows[:, self.ts_dataset.n_y:self.t_cols.index('insample_mask'), :self.input_size]
        insample_mask = windows[:, self.t_cols.index('insample_mask'), :self.input_size]

        outsample_y = windows[:, :self.ts_dataset.n_y, self.input_size:]
        outsample_x = windows[:, self.ts_dataset.n_y:self.t_cols.index('insample_mask'), self.input_size:]
        outsample_mask = windows[:, self.t_cols.index('outsample_mask'), self.input_size:]

        insample_mask = insample_mask.unsqueeze(1)
        outsample_mask = outsample_mask.unsqueeze(1)
        insample_mask = t.ones(insample_y.shape) * insample_mask
        outsample_mask = t.ones(outsample_y.shape) * outsample_mask
