...
def get_experiment_space(args):
    ...
    if args.space == 'nbeats_x_interpretable':
        space = {'initialization':  hp.choice('initialization', ['orthogonal','he_uniform',
                ...
                'n_val_weeks': hp.choice('n_val_weeks', [args.n_val_weeks])}
    if args.space == 'dnn':
        space = {'initialization':  hp.choice('initialization', ['lecun_normal','glorot_uniform','orthogonal',
                                                                 'he_uniform','he_normal','glorot_normal']),
                'activation': hp.choice('activation', ['relu','tanh','lrelu','softplus','selu','prelu','sigmoid']),
                'stack_types': hp.choice('stack_types', [ ['identity'] ]),
                'n_blocks': hp.choice('n_blocks', [ [1] ]),
                'n_layers': hp.choice('n_layers', [ [2] ]),
                'n_hidden_1': hp.quniform('n_hidden_1', 50, 500, 1),
                'n_hidden_2': hp.quniform('n_hidden_2', 50, 500, 1),
